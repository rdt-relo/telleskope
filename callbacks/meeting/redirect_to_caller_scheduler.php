...
<head>
	...
	<title>Generating Meeting links - 2/2</title>
</head>
<body>
    ...
    elseif (str_starts_with($_GET['code'],'ZJSON:')) {
        $zjson = str_replace('ZJSON:', '', $_GET['code']);
        $json = gzinflate(base64_decode(strtr($zjson, '._-', '+/=')));
    ?>
    <p style="color:green;">Getting meeting link<span id="status1"> done, redirecting</span></p>
    <script>
        window.onload = function () {
            window.opener.document.getElementById("scheduler_meetinglinks").value = '<?=$json?>';
            window.close();
        }
    </script>
    <?php } ?>
