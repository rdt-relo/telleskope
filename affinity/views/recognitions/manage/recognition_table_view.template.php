<table id="recognition_list" class="table table-hover display compact" summary="This table display the list of recognition of a group">
    <thead>
        <tr>
            <th width="38%" class="color-black" scope="col"><?= gettext("Person Recognized");?></th>
            <th width="38%" class="color-black" scope="col"><?= gettext("Recognized by");?></th>
            <th width="20%" class="color-black" scope="col"><?= gettext("Recognized On");?></th>
...
<script>
    $(document).ready(function() {
        ...
        var dtable = 	$('#recognition_list').DataTable( {
            serverSide: true,
            processing: true,
            bFilter: true,
            bInfo : false,
            ...
            ajax:{
                url :"ajax_recognition.php?getManageRecognitionsList=<?= $enc_groupid; ?>&isactive="+byState+"&year="+byYear+'&groupStateType='+groupStateType+'&groupState='+groupState, // json datasource
                type: "POST",  // method  , by default get
                error: function(data){  // error handling
                    $(".table-grid-error").html("");
                    $("#recognition_list").append('<tbody class="table-grid-error"><tr><th colspan="6">No data found!</th></tr></tbody>');
        ...
        .bind("input", function(e) {
         ...
         screenReadingTableFilterNotification('#recognition_list',dtable);
    });
$('#recognition_list').on( 'length.dt', function ( e, settings, len ) {
