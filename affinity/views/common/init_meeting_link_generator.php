...
function generateMeetingLinkOptionsHTML()
{
    ...
    if (in_array(true, array_values($_COMPANY->getAppCustomization()['event']['meeting_links'])) || !empty($my_link)) { // Only show link options if atleast one of the options is enabled
        ...
        if ($_COMPANY->getAppCustomization()['event']['meeting_links']['msteams']) {
            ...
            $show_seperator = true;
            echo '    <a id="TeamsSignIn" tabindex="0" role="button" onclick="TeamsSignIn()" style="cursor:pointer; padding: 0 12px;"><img alt="Teams Sign In" src="../image/microsoft-team.png" height="22"><span style="padding: 0 3px;">Teams</span></a>';
        }
        if ($_COMPANY->getAppCustomization()['event']['meeting_links']['gmeet']) {
            ...
            $show_seperator = true;
            echo '    <a id="GoogleSignIn" role="button" tabindex="0" onclick="GoogleSignIn()" style="cursor:pointer; padding: 0 12px;"><img alt="Google Sign In" src="../image/google-meet.png" height="22"><span style="padding: 0 3px;">Google Meet</span></a>';
        }
        if ($_COMPANY->getAppCustomization()['event']['meeting_links']['zoom']) {
            ...
            $show_seperator = true;
            echo '    <a id="ZoomSignIn" role="button" tabindex="0" onclick="ZoomSignIn()" style="cursor:pointer;padding: 0 12px;"><img alt="Zoom Sign In" src="../image/meetingzoom.png" height="22"><span style="padding: 0 3px;">Zoom</span></a>';
        }
        echo '    </div>';
        echo '</div>';
    }
