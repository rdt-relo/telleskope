<div class="col-md-12">
    <div class="row">
      <div class="col-12">
        <h2 id="managePost"><?php echo sprintf(gettext("Manage %s"),Post::GetCustomName(true)) ?> - 
        <?php 
      ...
      if ($_USER->canCreateContentInGroupSomething($groupid)) {
          ...
          if (Disclaimer::IsDisclaimerAvailable(Disclaimer::DISCLAIMER_HOOK_TRIGGERS['POST_CREATE_BEFORE'])) {
              $callOtherMethod = base64_url_encode(json_encode(array("method" => "newAnnouncement", "parameters" => array($encGroupId, $isGlobal)))); // base64_encode for prevent js parsing error
              $hookid = $_COMPANY->encodeId(Disclaimer::DISCLAIMER_HOOK_TRIGGERS['POST_CREATE_BEFORE']);
              $encoded0 = $_COMPANY->encodeId(0);
              $on_create_function = "loadDisclaimerByHook('{$hookid}', '{$encoded0}', '0', '{$callOtherMethod}')";
          } else {
              $on_create_function = "newAnnouncement('{$encGroupId}', '{$isGlobal}')";
          }
          $newbtn = '<button id="createPostBtn" class="btn btn-primary dropdown-toggle roster" type="button" onclick="' . $on_create_function . '">' . addslashes(sprintf(gettext('Create %s'), Post::GetCustomName(false))) . '</button>';
      ...
      ?>
        <div class="col-md-12">
            <div class="col-md-4">
                <div class="form-group col-md-12 ">
                    <label style="font-size:small;"><?= sprintf(gettext("Filter by %s state"), Post::GetCustomName());?></label>
                    <select aria-label="<?= sprintf(gettext("Filter by %s state"), Post::GetCustomName());?>" class="form-control" onchange="filterAnnouncements('<?= $encGroupId; ?>');" id="filterByState" style="font-size:small;border-radius: 5px;">
            ...
            <div class="col-md-4">
                ...
                <?php if($groupid>0){ 
                  ...
                  ?>
                    ...
                    <select id="filterByGroup"  onchange="filterAnnouncements('<?= $encGroupId; ?>');" style="font-size:small;border-radius: 5px;" class="form-control" >
            ...
            <div class="col-md-4 ">
                <div class="form-group col-md-12 " style="float:right !important;">
                    ...
                    <select aria-label="<?= gettext("Filter by calendar year");?>" class="form-control" id="filterByYear" onchange="filterAnnouncements('<?= $encGroupId; ?>'
        ...
        <div class="col-md-12 mt-2">
            <div class="clearfix"></div>
            <div class="table-responsive" id="announcementTable">
                <?php
                    include(__DIR__ . "/group_announcement_table.template.php");
                ?>
            </div>
        </div>
    </div>
