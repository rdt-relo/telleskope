<div id="theReportModal" class="modal fade" >
	<div aria-label="<?= gettext("Download Newsletters Report");?>" class="modal-dialog modal-lg" aria-modal="true" role="dialog">  
		<div class="modal-content">
			<div class="modal-header">
                <h2 class="modal-title text-center" id="newsletter_report_title"><?= gettext("Download Newsletters Report");?></h2>
                <button aria-label="<?= gettext("close");?>"  id="btn_close" type="button" class="close" data-dismiss="modal">&times;</button>
			</div>
            <div class="modal-body">
                <div class="col-md-12 mt-3 text-center">
                    <form class="form-horizontal" id="reportsForm" action="ajax_reports?downloadNewslettersReport=1" method="post" role="form" style="display: block;">
                    <input type="hidden" name="csrf_token" value="<?= Session::GetInstance()->csrf; ?>">
                                ...
                                <fieldset>
                                    <legend style="font-size: 1.2rem;"><?= gettext("Select Fields"); ?></legend>
                                        <div class="mb-2 text-sm">
                                            <a role="button" href="javascript:void(0)" class="link_show" onclick="checkUncheckAllCheckBoxes('newsletterOptionsMultiCheck',true)"> <?= gettext("Select All");?></a> | <a role="button" href="javascript:void(0)" class="link_show" onclick="checkUncheckAllCheckBoxes('newsletterOptionsMultiCheck',false)"> <?= gettext("Deselect All");?></a>
                                        </div>
                                        ...
                                        ?>
                                        <input aria-label="<?= $value; ?>" class="newsletterOptionsMultiCheck" type="checkbox" name="<?= $key; ?>" value="<?= $key; ?>" <?= $checked; ?>>&emsp;<?= $value; ?><br>
                                        <?php } ?>
                                    </fieldset>
                                </div>
                            </div>
                        ...
                        <div class="form-group mt-3">
                            <button type="submit" name="submit" id="submit_action_button" class="btn btn-primary"><?= gettext("Download Newsletters Report");?></button>
                        </div>
