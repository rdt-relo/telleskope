...
Class ReportAnnouncementsInternal extends Report
{
...
//            'clickscount' => 'Total number of of Clicks',
    ...
    protected static function GetReportType() : int { return self::REPORT_TYPE_ANNOUNCEMENT_INTERNAL;}
    ...
    protected function _generateReport($file_h, string $delimiter = ",", string $enclosure = '"', string $escape_char = "\\"): void
    {
        ...
        $select = "SELECT * FROM `post` WHERE `companyid` = '{$this->cid()}' AND zoneid='{$_ZONE->id()}' AND isactive > 0 ORDER BY postid DESC {$this->policy_limit}";
