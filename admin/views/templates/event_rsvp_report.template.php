	...
	<div class="modal-dialog modal-lg">  
		<div class="modal-content">
			<div class="modal-header">
                <h4 class="modal-title text-center" id="rsvp_report_title"><?= gettext("Download Event RSVP Report");?> <i class="fa fa-info-circle info-black" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="<?= gettext('Below, choose the filters you wish to include in the downloaded report.'); ?>"></i></h4>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
			</div>
            <div class="modal-body">
                <div class="col-md-12 mt-3 text-center">
                    <form class="form-horizontal" action="ajax.php?download_rsvp_report=1" method="post" role="form" style="display: block;">
                            ...
                            <div class="row pt-3">
                                <div class="col-sm-6 text-left">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="s_group[]" id="s_group2" multiple size="6">
                                                        <!-- <option value="<?= $_COMPANY->encodeId(0) ?>">All Groups</option> -->
                                                        <option value="<?= $_COMPANY->encodeId(0) ?>" selected><?= 'Global Events' ?></option>
                                                        <?php foreach ($group as $g) { ?>
                                                        <option value="<?= $_COMPANY->encodeId($g['groupid']) ?>" selected><?= $g['groupname']; ?></option>
                                                        <?php } ?>
                                                    </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="s_group">From Date</label>
                                        <input type="text" class="form-control" id="start_date2" name="startDate" value="<?= $startDate; ?>" readonly placeholder="YYYY-MM-DD">
                                    </div>
                                    <div class="form-group">
                                        <label for="s_group">To Date</label>
                                        <input type="text" class="form-control" id="end_date2"  name="endDate" value="" readonly placeholder="YYYY-MM-DD">
                                    </div>
                                    <div class="form-group">
...
jQuery(function() {
    ...
    jQuery( "#start_date2" ).datepicker({
        ...
        onSelect: function (selectedDate) {
                ...
                $("#end_date2").datepicker("option", 'minDate', orginalDate);
                $("#end_date2").datepicker("option", 'maxDate', monthsAddedDate);
            }
	});
});
$(document).ready(function () {
    ...
    $("#end_date2").datepicker("option", 'minDate', todayDate);
    $("#end_date2").datepicker("option", 'maxDate', monthsAddedDate);
    $("#start_date2").datepicker('setDate', todayDate);
    $("#end_date2").datepicker('setDate', monthsAddedDate);
});
</script>
<script>
    $(function () {
