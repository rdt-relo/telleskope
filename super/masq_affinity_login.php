...
Auth::CheckPermission(Permission::MasqAffinity);
...
$check2 = $_SUPER_ADMIN->super_get("SELECT `subdomain`,`aes_suffix` FROM `companies` WHERE `companyid`='{$companyid}' and `isactive`='1'");
	...
	if (count($check2)) {
		...
		$aes_prefix = substr(TELESKOPE_USERAUTH_AFFINITY_KEY,2,22);
		...
		$uri = 'https://'.$check2[0]['subdomain'].'.affinities.io/1/affinity/login_masq_affinity?HqsM4kCF5z='.$encrypted_token;
		Logger::Log("Super Admin - 0|{$superid}|MasqAffinity into {$companyid}|{$userid}", Logger::SEVERITY['INFO']);
		header("location:  ".$uri);
		exit();
	}
}
